...
function gameOver() {
  ...
  $("h1").text("Konec hry, stisknutím libovolné klávesy nebo kliknutím na jedno z tlačítek restartujte.");
  clearLog(levelLog);
  clearLog(playerLog);
...
function check() {
  ...
  if (playerLog.length > 1 && levelLog.length === playerLog.length) {
    clearLog(playerLog);
    setTimeout(function() {
      $("h1").text("Úroveň " + levelLog.length);
    }, 1000);
    setTimeout(function() {
      simonSays();
    }, 2000);
...
$(".btn").on("click", function() {
    if (levelLog.length > "1" && clickLock[0] == "start") {
      ...
      check ();
    } else if (document.querySelector("h1").textContent == "Konec hry, stisknutím libovolné klávesy nebo kliknutím na jedno z tlačítek restartujte.") {
      startGame();
    }
    if (document.querySelector("h1").textContent == "Začněte stisknutím klávesy nebo kliknutím na tlačítko") {
      startGame();
    }
});
